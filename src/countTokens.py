...
icd10_train = split[split['split'] == 'test']
...
prompts = [(note_id, tokenizer.apply_chat_template(prompt, tokenize=True, add_generation_prompt=True)) for note_id, prompt in prompts]

max_seq_length = 0
mean_seq_length = 0
num_mag_8k = 0
lunghezze = []
for note_id, text in prompts:
    lunghezza = len(text)
...
mean_seq_length /= len(prompts)
print(f"Max sequence length: {max_seq_length}")
print(f"Mean sequence length: {mean_seq_length}")
print(f"Num > 8k: {num_mag_8k}")

notes["lenght"] = lunghezze

notes.to_feather(DATA_DIR + "test_set_lenghts.feather")
...
#    }
#    with open(OUTPUT_DIR + 'testEnts.jsonl', 'a') as f:
